...
public class MainActivity extends AppCompatActivity implements Runnable {
    ...
    private FloatingActionButton fabForward; // поле кнопки перемоткт вперёд
    ...
    private FloatingActionButton fabNext; //поле кнопки переключения трека
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromTouch) {
                ...
                if (progress > 0 && mediaPlayer != null && !mediaPlayer.isPlaying()) { // если mediaPlayer не пустой и mediaPlayer не воспроизводится
                    pauseMediaPlayer(); // остановка MediaPlayer
        ...
        fabForward.setOnClickListener(listener); //сождание слушателя нажатия кнопки fabForward (перемотка вперд)
        fabRepeat.setOnClickListener(listener); // создание слушателя нажатия кнопки fabRepeat (повтора)
        fabNext.setOnClickListener(listener); //создание слушателя нажатия кнопки fabNext
        ...
        public void onClick(View view) {
            ...
            switch (view.getId()) {
                ...
                case R.id.fabRepeat:
                    ...
                    break;
                case R.id.fabNext:
                    playSong("george_thorogood_bad_to_the_bone_mix.mp3");
                    break;
            }
        }
    };
    ...
    public void playSong() {
        try { // обработка исключения на случай отстутствия файла
            ...
            if (!wasPlaying) {
                ...
                if (mediaPlayer != null && !mediaPlayer.isPlaying()) { // если mediaPlayer не пустой и не воспроизводится
                    mediaPlayer.start();
                    ...
                    AssetFileDescriptor descriptor = getAssets().openFd("Danny_Deluxe-Gustas.mp3");
    ...
    public void playSong(String NameFail) {
        try { // обработка исключения на случай отстутствия файла
            if (mediaPlayer != null && mediaPlayer.isPlaying()) { // если mediaPlayer не пустой и mediaPlayer воспроизводится
                pauseMediaPlayer(); // остановка MediaPlayer
                wasPlaying = true; // инициализация значения запуска аудио-файла
            ...
            if (!wasPlaying) {
                ...
                mediaPlayer.reset() ;
                // альтернативный способ считывания файла с помощью файлового дескриптора
                AssetFileDescriptor descriptor = getAssets().openFd(NameFail);
                // запись файла в mediaPlayer, задаются параметры (путь файла, смещение относительно начала файла, длина аудио в файле)
                mediaPlayer.setDataSource(descriptor.getFileDescriptor(), descriptor.getStartOffset(), descriptor.getLength());
